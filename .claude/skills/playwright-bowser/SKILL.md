...
# "test the checkout flow on app" → -s=keu-checkout
# "scrape data from app"          → -s=keu-scrape
# "UI test the login page"         → -s=keu-login-test

playwright-cli -s=keu-test open http://localhost:3000 --persistent
playwright-cli -s=keu-test snapshot
playwright-cli -s=keu-test click e12
```
